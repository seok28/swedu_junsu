...
const showAllLadderButton = document.querySelector("#showAllLadderButton");
...
let ladderPattern = [];
let ladderResults = [];
let revealedLadderIndexes = new Set();
let activeLadderIndex = null;
let activeLadderStep = 0;
let ladderStepTimerId = null;
...
function renderLadderPlaceholder() {
  ...
  updateLadderSummary();
  ladderMessage.textContent = "공통 설정에서 참여 인원과 뽑을 인원 수를 정해 주세요.";
}
...
function renderLadderPreview() {
  ...
  ladderPattern = generateLadderPattern(participants.length);
  ladderBoard.innerHTML = buildLadderBoard(participants, [], false, ladderPattern);
  ladderMessage.textContent = "사다리 시작을 누르면 새 사다리가 만들어집니다.";
}
...
function runLadder() {
  ...
  ladderPattern = generateLadderPattern(participants.length);
  ladderResults = resolveLadderResults(participants, pickCount, ladderPattern);
  revealedLadderIndexes = new Set();
  activeLadderIndex = null;
  showAllLadderButton.disabled = false;
  updateShareButton();
  ladderBoard.innerHTML = buildLadderBoard(participants, ladderResults, false, ladderPattern);
  ladderMessage.textContent = "이름을 클릭하면 해당 경로가 진행됩니다. 전체 결과보기로 한 번에 확인할 수도 있습니다.";
}

function resetLadderState() {
  clearTimeout(ladderStepTimerId);
  ladderStepTimerId = null;
  ladderResults = [];
  revealedLadderIndexes = new Set();
  activeLadderIndex = null;
  activeLadderStep = 0;
  showAllLadderButton.disabled = true;
}

function revealLadderResult(index) {
  if (isLadderRunning || !ladderResults.length || revealedLadderIndexes.has(index)) return;
  ...
  activeLadderIndex = index;
  activeLadderStep = 0;
  startLadderButton.disabled = true;
  showAllLadderButton.disabled = true;
  ladderBoard.innerHTML = buildLadderBoard(participants, ladderResults, true, ladderPattern);
  ladderMessage.textContent = `${result.name} 사다리를 타는 중입니다.`;
  advanceLadderStep(index);
}

function advanceLadderStep(index) {
  const result = ladderResults[index];
  const participants = getParticipants();
  const stepCount = Math.max(1, result?.path?.usedRungs?.length || 0);

  ladderStepTimerId = setTimeout(() => {
    activeLadderStep += 1;
    ladderBoard.innerHTML = buildLadderBoard(participants, ladderResults, true, ladderPattern);
    ladderMessage.textContent = `${result.name} 사다리 진행 중 ${Math.min(activeLadderStep, stepCount)} / ${stepCount}`;

    if (activeLadderStep < stepCount) {
      advanceLadderStep(index);
      return;
    }
    ...
    ladderStepTimerId = setTimeout(() => {
      ...
      revealedLadderIndexes.add(index);
      activeLadderIndex = index;
      activeLadderStep = stepCount;
      startLadderButton.disabled = false;
      showAllLadderButton.disabled = false;
      ladderBoard.innerHTML = buildLadderBoard(participants, ladderResults, false, ladderPattern);
      ladderMessage.textContent = `${result.name}님은 ${result.selected ? "당첨" : "통과"}입니다.`;
      if (revealedLadderIndexes.size === ladderResults.length) {
        completeLadderResults("전체 결과를 확인했습니다.");
      }
    }, 450);
  }, 320);
}
...
function showAllLadderResults() {
  if (!ladderResults.length || isLadderRunning) return;
  ladderResults.forEach((_, index) => revealedLadderIndexes.add(index));
  activeLadderIndex = null;
  completeLadderResults("전체 결과를 표시했습니다.");
}

function completeLadderResults(message) {
  clearTimeout(ladderStepTimerId);
  ladderStepTimerId = null;
  const participants = getParticipants();
  const winners = ladderResults.filter((result) => result.selected).map((result) => result.name);
  renderWinners(winners);
  lastPickerResult = savePickerHistory(winners, participants.length, "사다리타기");
  renderPickerHistory();
  updateShareButton();
  ladderBoard.innerHTML = buildLadderBoard(participants, ladderResults, false, ladderPattern);
  drawMachine.innerHTML = winners.length
    ? winners.map((name) => `<span>${escapeHtml(name)}</span>`).join("")
    : "<span>READY</span>";
  ladderMessage.textContent = message;
  showAllLadderButton.disabled = true;
}
...
function generateLadderPattern(laneCount) {
  if (laneCount < 2) return [];

  const rowCount = Math.min(8, Math.max(5, laneCount + 1));
  const ladderHeight = 230;
  const rows = [];
  for (let row = 0; row < rowCount; row += 1) {
    const usedColumns = new Set();
    const rungCount = Math.max(1, Math.ceil((laneCount - 1) / 2));
    for (let step = 0; step < rungCount; step += 1) {
      const column = randomInt(0, laneCount - 2);
      if (usedColumns.has(column) || usedColumns.has(column - 1) || usedColumns.has(column + 1)) continue;
      usedColumns.add(column);
      rows.push({
        column,
        top: 47 + Math.round(ladderHeight * ((row + 1) / (rowCount + 1))),
      });
    }
  }
  return rows;
...
function resolveLadderResults(participants, pickCount, pattern) {
  const winningLanes = new Set(shuffleItems(participants.map((_, index) => index)).slice(0, pickCount));
  return participants.map((name, index) => {
    const path = traceLadderPath(index, pattern);
    return {
      name,
      selected: winningLanes.has(path.finalLane),
      path,
    };
  });
}

function traceLadderPath(startIndex, pattern) {
  let currentLane = startIndex;
  const usedRungs = [];
  const lanes = [startIndex];
  const sortedPattern = [...pattern].sort((a, b) => a.top - b.top);

  sortedPattern.forEach((rung) => {
    if (rung.column === currentLane) {
      usedRungs.push(getLadderRungKey(rung));
      currentLane += 1;
      lanes.push(currentLane);
      return;
    }

    if (rung.column === currentLane - 1) {
      usedRungs.push(getLadderRungKey(rung));
      currentLane -= 1;
      lanes.push(currentLane);
    }
...
function getLadderRungKey(rung) {
  return `${rung.column}-${rung.top}`;
}

function buildLadderBoard(participants, results, isRunning = false, pattern = ladderPattern) {
  const resultMap = new Map(results.map((result) => [result.name, result.selected]));
  const visibleResultIndexes = new Set(revealedLadderIndexes);
  const pathResultIndexes = new Set(revealedLadderIndexes);
  if (activeLadderIndex !== null) {
    pathResultIndexes.add(activeLadderIndex);
  }
  const selectedRungs = new Set(
    results.flatMap((result, index) => {
      if (!result.path || !pathResultIndexes.has(index)) return [];
      if (activeLadderIndex === index && isRunning) {
        return result.path.usedRungs.slice(0, activeLadderStep);
      }
      return result.path.usedRungs;
    }),
  );
  const activeLanes = new Set();
  if (activeLadderIndex !== null && results[activeLadderIndex]?.path) {
    const laneLimit = isRunning ? activeLadderStep + 1 : results[activeLadderIndex].path.lanes.length;
    results[activeLadderIndex].path.lanes.slice(0, laneLimit).forEach((lane) => activeLanes.add(lane));
  }
  const rungsByColumn = new Map();
  pattern.forEach((rung) => {
    const current = rungsByColumn.get(rung.column) || [];
    current.push(rung);
    rungsByColumn.set(rung.column, current);
  });
    ...
    <div class="ladder-grid${isRunning ? " is-running" : ""}" style="--ladder-count: ${participants.length}">
      ...
      ${participants
        .map((name, index) => {
          ...
          const isVisible = visibleResultIndexes.has(index);
          const isActive = activeLadderIndex === index;
          const isPathLane = activeLanes.has(index);
          const resultText = hasResult && isVisible ? (selected ? "당첨" : "통과") : "?";
          return `
            <div class="ladder-line ${isVisible && selected ? "is-selected" : ""} ${isPathLane ? "is-path" : ""}">
              <button class="ladder-name ${isActive ? "is-active" : ""}" type="button" data-ladder-index="${index}" ${!results.length || isRunning ? "disabled" : ""}>${escapeHtml(name)}</button>
              <span class="ladder-path"></span>
              ${(rungsByColumn.get(index) || [])
                .map((rung) => {
                  const isSelectedRung = selectedRungs.has(getLadderRungKey(rung));
                  return `<span class="ladder-rung ${isSelectedRung ? "is-selected" : ""}" style="--rung-top: ${rung.top}px"></span>`;
                })
                .join("")}
              <em>${resultText}</em>
            </div>
...
startPickerButton.addEventListener("click", runPicker);
startLadderButton.addEventListener("click", runLadder);
showAllLadderButton.addEventListener("click", showAllLadderResults);
...
ladderBoard.addEventListener("click", (event) => {
  const button = event.target.closest("[data-ladder-index]");
  if (!button) return;
  revealLadderResult(Number(button.dataset.ladderIndex));
});
