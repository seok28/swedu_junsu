...
function runPicker(event) {
  ...
  setTimeout(() => {
    ...
    savePickerHistory(winners, participants.length);
    renderPickerHistory();
...
function savePickerHistory(winners, totalCount) {
    ...
    .map((entry) => {
      ...
      return `
        <div class="history-item">
          ...
          <span>${escapeHtml(entry.createdAt)} · 전체 ${Number(entry.totalCount) || 0}명</span>
        </div>
      `;
    })
    .join("");
