(() => {
  ...
  function renderBoard() {
    ...
    participants.forEach((name, index) => {
      ...
      html += `
        <g>
          <text class="name-label" x="${x}" y="24" text-anchor="middle">${app.escapeHtml(name.trim() || `참가자 ${index + 1}`)}</text>
          <line class="vertical" x1="${x}" y1="${topY}" x2="${x}" y2="${bottomY}"></line>
          <text class="result-label" x="${x}" y="402" text-anchor="middle">${app.escapeHtml(visibleResult || "?")}</text>
        </g>
      `;
    });
